...
/**
 * element.querySelectorAllWithShadowDOM but also considers owned tree
 * @param element
 * @param selectors
 */
...
function querySelectorAllSubtree(element, selectors) {
  var elements = arrayFrom(element.querySelectorAllWithShadowDOM(selectors));
  queryIdRefs(element, "aria-owns").forEach(function (root) {
    // babel transpiles this assuming an iterator
    elements.push.apply(elements, arrayFrom(root.querySelectorAllWithShadowDOM(selectors)));
  });
  return elements;
}
...
function getLabels(element) {
  ...
  return arrayFrom(document.querySelectorAllWithShadowDOM("label")).filter(function (label) {
...
function checkContainerType(container) {
  if (!container || !(typeof container.querySelector === 'function') || !(typeof container.querySelectorAllWithShadowDOM === 'function')) {
...
function getRealLabels(element) {
  ...
  var labels = element.ownerDocument.querySelectorAllWithShadowDOM('label');
...
function getLabels$1(container, element, _temp) {
  ...
  var labelsId = element.getAttribute('aria-labelledby') ? element.getAttribute('aria-labelledby').split(' ') : [];
  return labelsId.length ? labelsId.map(function (labelId) {
    ...
    var labelledFormControl = Array.from(label.querySelectorAllWithShadowDOM('button, input, meter, output, progress, select, textarea')).filter(function (formControlElement) {
...
function queryAllByAttribute(attribute, container, text, _temp) {
  ...
  return Array.from(container.querySelectorAllWithShadowDOM("[" + attribute + "]")).filter(function (node) {
...
function queryAllLabels(container) {
  return Array.from(container.querySelectorAllWithShadowDOM('label,input')).map(function (node) {
    return {
      node: node,
...
function queryAllByLabelText(container, text, _temp2) {
  ...
  var matchingLabelledElements = Array.from(container.querySelectorAllWithShadowDOM('*')).filter(function (element) {
...
function queryAllByText(container, text, _temp) {
  ...
  return [].concat(baseArray, Array.from(container.querySelectorAllWithShadowDOM(selector))).filter(function (node) {
...
function queryAllByDisplayValue(container, value, _temp) {
  ...
  return Array.from(container.querySelectorAllWithShadowDOM("input,textarea,select")).filter(function (node) {
...
function queryAllByAltText(container, alt, _temp) {
  ...
  return Array.from(container.querySelectorAllWithShadowDOM('img,input,area')).filter(function (node) {
...
function queryAllByTitle(container, text, _temp) {
  ...
  return Array.from(container.querySelectorAllWithShadowDOM('[title], svg > title')).filter(function (node) {
...
function queryAllByRole(container, role, _temp) {
  ...
  return Array.from(container.querySelectorAllWithShadowDOM('*')).filter(function (node) {
